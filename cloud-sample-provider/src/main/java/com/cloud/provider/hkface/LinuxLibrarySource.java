...
import com.cloud.provider.hkface.linux.HCNetSDK;
...
public class LinuxLibrarySource implements LibrarySource {
    ...
    public boolean checkFaceFaceCapabilities(String arg) {
        NativeLong userId = loginCache.get(arg);
    ...
    public class FMSGCallBack implements HCNetSDK.FMSGCallBack {
        ...
        public void invoke(NativeLong lCommand, HCNetSDK.NET_DVR_ALARMER pAlarmer, Pointer pAlarmInfo, int dwBufLen, Pointer pUser) {
            ...
            switch (lCommand.intValue()) {
                case HCNetSDK.COMM_UPLOAD_FACESNAP_RESULT: //实时人脸抓拍上传，人脸识别结果上传
...
//                    eventParam.setGlass(CommonUtil.byteToInt(strFaceSnapMatch.struSnapInfo.byGlasses));
                    LOGGER.info("sex:{}", CommonUtil.byteToInt(strFaceSnapMatch.struSnapInfo.bySex));
//                    eventParam.setSex(CommonUtil.byteToInt(strFaceSnapMatch.struSnapInfo.bySex));
                    LOGGER.info("ageGroup:{}", CommonUtil.byteToInt(strFaceSnapMatch.struSnapInfo.byAgeGroup));
//                    eventParam.setAgeGroup(CommonUtil.byteToInt(strFaceSnapMatch.struSnapInfo.byAgeGroup));
...
//                    eventParam.setXsd(strFaceSnapMatch.fSimilarity);
                    ...
                    if ((strFaceSnapMatch.struBlackListInfo.dwFDIDLen > 0) && (strFaceSnapMatch.struBlackListInfo.pFDID != null)) {
                        ...
                        String FDID = new String(FDIDbytes).trim();
//                        eventParam.setNvrFDId(FDID);
//                        LogUtil.info(logger, "人脸库ID:{}", FDID);
                    }
                    //获取人脸图片ID
                    byte[] PIDbytes;
                    if ((strFaceSnapMatch.struBlackListInfo.dwPIDLen > 0) && (strFaceSnapMatch.struBlackListInfo.pPID != null)) {
                        ...
                        String faceId = new String(PIDbytes).trim();
//                        eventParam.setNvrFaceId(faceId);
//                        LogUtil.info(logger, "人脸图片ID:{}", faceId);
//                        NVRFace nvrFace = nvrFaceMapper.selectByPrimaryKey(faceId);
//                        if(nvrFace != null && nvrFace.getObsId() != null){
//                            FaceLabelExample faceLabelExample = new FaceLabelExample();
//                            FaceLabelExample.Criteria criteria = faceLabelExample.createCriteria();
//                            criteria.andObsIdEqualTo(nvrFace.getObsId());
//                            FaceLabel faceLabel = faceLabelMapper.selectByExample(faceLabelExample).get(0);
//                            eventParam.setName(faceLabel.getName());
//                        }
                    }
                    break;
                default:
